...
bool operator==(const CustomClass& lhs, const CustomClass& rhs) { return lhs.value == rhs.value; }

class CustomException : public std::exception
{};

void throwCustomException() { throw CustomException{}; }

register_scenario(H2OFastTests_Tests) {
    auto epsf_v = 1e-5f;
    auto epsd_v = 1e-5;

    set_up([]() { std::cout << "Setup" << std::endl; });

    tear_down([]() { std::cout << "Teardown" << std::endl; });

    skip_test("Test skip", "Assert::AreEqual(double, tolerance = 1e-5)",
              [epsd_v]() { AssertThat(0.).isEqualTo(epsd_v, epsd_v, "Expect 0. == 1e-5"); });

    add_test("Assert::AreNotEqual(double, tolerance = 1e-5)",
             [epsd_v]() { AssertThat(0.).isNotEqualTo(epsd_v * 10, epsd_v, "Expect 0. != 10e-5"); });

    add_test("Assert::AreEqual(float, tolerance = 1e-5)",
             [epsf_v]() { AssertThat(0.f).isEqualTo(epsf_v, epsf_v, "Expect 0.f == 1e-5f"); });

    add_test("Assert::AreNotEqual(float, tolerance = 1e-5)",
             [epsf_v]() { AssertThat(0.f).isNotEqualTo(epsf_v * 10, epsf_v, "Expect 0.f != 10e-5f"); });
    ...
    add_test("Assert::AreNotEqual(CustomClass)", []() {
        AssertThat(CustomClass{true})
        .isNotEqualTo(CustomClass{false}, "Expect CustomClass{ true } != CustomClass{ false}");
    });
    ...
    add_test("Assert::IsTrue(true)", []() { AssertThat(true).isTrue("Expect true == true"); });

    add_test("Assert::IsFalse(false)", []() { AssertThat(false).isFalse("Expect true != false"); });

    add_test("Assert::ExceptException<CustomException>", []() {
        AssertThat([]() { throw CustomException{}; }).expectException<CustomException>("Expect catch(CustomException)");
