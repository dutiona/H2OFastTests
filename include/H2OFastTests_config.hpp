...
#if _MSC_VER >= 1500
#define H2OFT_DISABLE_MSC_WARNINGS_PUSH_(warnings) __pragma(warning(push)) __pragma(warning(disable : warnings))
#define H2OFT_DISABLE_MSC_WARNINGS_POP_() __pragma(warning(pop))
#else
...
enum H2OFTColor { COLOR_DEFAULT, COLOR_RED, COLOR_BLUE, COLOR_GREEN, COLOR_YELLOW, COLOR_PURPLE, COLOR_CYAN };

#if H2OFT_OS_WINDOWS && !H2OFT_OS_WINDOWS_MOBILE && !H2OFT_OS_WINDOWS_PHONE && !H2OFT_OS_WINDOWS_RT
...
bool ShouldUseColor(bool stdout_is_tty) {
    ...
    if (H2OFT_color == "auto") {
        ...
        char*  buffer = nullptr;
        size_t sz     = 0;
        if (_dupenv_s(&buffer, &sz, "TERM") == 0 && buffer == nullptr) { return stdout_is_tty; }
        const std::string term = std::string{buffer};
#define FREE_BUFFER free(buffer);
#endif

        // On non-Windows platforms, we rely on the TERM variable.
        const bool term_supports_color = term == "xterm" || term == "xterm-color" || term == "xterm-256color" ||
                                         term == "screen" || term == "screen-256color" || term == "tmux" ||
                                         term == "tmux-256color" || term == "rxvt-unicode" ||
                                         term == "rxvt-unicode-256color" || term == "linux" || term == "cygwin";
...
#undef FREE_BUFFER
    ...
    return H2OFT_color == "yes" || H2OFT_color == "true" || H2OFT_color == "t" || H2OFT_color == "1";
...
#if H2OFT_OS_WINDOWS_MOBILE || H2OFT_OS_SYMBIAN || H2OFT_OS_ZOS || H2OFT_OS_IOS || H2OFT_OS_WINDOWS_PHONE || \
H2OFT_OS_WINDOWS_RT
    ...
    fflush(stdout);
    SetConsoleTextAttribute(stdout_handle, GetForegroundColorAttribute(color) | FOREGROUND_INTENSITY);
    vprintf(fmt, args);
